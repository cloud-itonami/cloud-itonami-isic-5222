(ns portauthority.operation
  "OperationActor -- one port/harbor-support-services coordination
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (PortAuthorityAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Port Authority Governor (:govern) and the rollout phase gate
  ...
  the decision to a human operator (port authority operator / harbor
  master). The approver resumes with `{:approval {:status :approved}}`
  (or :rejected). `:flag-navigation-safety-concern` ALWAYS reaches
  this node when the governor is clean -- see `portauthority.phase`."
...
(defn- op->store-effect
  "Map a coordination op to its store commit-effect key. The
  proposal's OWN :effect field stays the literal :propose throughout
  -- this mapping is an operation-layer concern, not something the
  advisor ever controls (governor check 2 enforces `:effect :propose`
  on the proposal independent of this)."
  [op]
  (case op
    :log-service-record             :service/log
    :schedule-port-operation        :schedule/propose
    :coordinate-maintenance         :maintenance/coordinate
    :flag-navigation-safety-concern :concern/record))
...
(defn- commit-record [request _context _proposal]
  {:effect (op->store-effect (:op request))
   :path   [(:target-id request)]})

(defn build
  "Compiles an OperationActor graph bound to `store` (any
  `portauthority.store/Store`).
  opts:
    :advisor      -- a `portauthority.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PortAuthorityAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Port Authority Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :target-id (:target-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :target-id (:target-id request) :by (:by approval)}]}
            {:disposition :hold
